...
Le fichier Compose est préparé mais doit être exécuté sur votre serveur. Le service nécessite HTTPS public pour Telegram, DNS, token et ID : un simple hébergement de fichiers statiques ne suffit pas. Aucun serveur ni domaine n’est acheté ou créé par ce dépôt. Le code Python s’exécute aussi hors Docker sur Linux : exporter les variables nécessaires, donner un chemin inscriptible à `DATABASE_PATH`, puis `python -m app.server` derrière un proxy HTTPS.
